...
fs::path cs2gsi::generate_gsi_config(const std::string &name, const std::string &uri, const fs::path &steam_path) {
    fs::path cfg_dir = steam_path / "steamapps/common/Counter-Strike Global Offensive/game/csgo/cfg";
    ...
    std::ofstream f(out);
    if (!f)
        throw std::runtime_error("Could not open file for writing: " + out.string());
    f << gsi_config_body(name, uri);
    if (!f.good())
        throw std::runtime_error("Write failed: " + out.string());
