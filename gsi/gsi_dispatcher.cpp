...
namespace cs2gsi {
    ...
    ComponentObserver<Grenade> GSIDispatcher::grenade(const std::string &grenade_id) {
        return {[this](SubId id) { unsubscribe(id); },
    ...
    void GSIDispatcher::dispatch(const nlohmann::json &json) {
        ...
        if (state.player)
            prev_player_ = state.player;
        if (state.map)
            prev_map_ = state.map;
        if (state.round)
            prev_round_ = state.round;
        if (state.bomb)
            prev_bomb_ = state.bomb;
        if (state.all_players)
            prev_all_players_ = state.all_players;
        if (state.grenades)
            prev_grenades_ = state.grenades;
        if (state.phase_countdowns)
            prev_phase_countdowns_ = state.phase_countdowns;

        lock.unlock();

        if (state != old_gs)
            for (auto &[id, cb]: gs_subs)
                cb(old_gs, state);

        if (state.player && *state.player != old_p)
            for (auto &[id, cb]: p_subs)
                cb(old_p, *state.player);

        if (state.map && *state.map != old_m)
            for (auto &[id, cb]: m_subs)
                cb(old_m, *state.map);

        if (state.round && *state.round != old_r)
            for (auto &[id, cb]: r_subs)
                cb(old_r, *state.round);

        if (state.bomb && *state.bomb != old_b)
            for (auto &[id, cb]: b_subs)
                cb(old_b, *state.bomb);

        if (state.all_players && *state.all_players != old_ap)
            for (auto &[id, cb]: ap_subs)
                cb(old_ap, *state.all_players);

        if (state.grenades && *state.grenades != old_gr)
            for (auto &[id, cb]: gr_subs)
                cb(old_gr, *state.grenades);
        ...
        if (state.phase_countdowns && *state.phase_countdowns != old_pc)
            for (auto &[id, cb]: pc_subs)
                cb(old_pc, *state.phase_countdowns);
    }
} // namespace cs2gsi
