...
static const char *activity_str(PlayerActivity a) {
    switch (a) {
        case PlayerActivity::Playing:
            return "Playing";
        case PlayerActivity::Spectating:
            return "Spectating";
        case PlayerActivity::Menu:
            return "Menu";
        default:
            return "Unknown";
    }
...
int main() {
    ...
    dispatcher.player().on(&Player::activity, [](PlayerActivity old_a, PlayerActivity new_a) {
        std::cout << "[activity] " << activity_str(old_a) << " -> " << activity_str(new_a) << "\n";
    });
    ...
    dispatcher.player().on(&Player::team, [](PlayerTeam old_t, PlayerTeam new_t) {
        std::cout << "[team] " << team_str(old_t) << " -> " << team_str(new_t) << "\n";
    });
    ...
    dispatcher.on_player([](const Player &old_p, const Player &new_p) {
        for (auto &[slot, w]: new_p.weapons) {
            ...
            if (!was_active)
                std::cout << "[weapon] " << w.name << " (" << weapon_type_str(w.type) << ")\n";
        }
    });
