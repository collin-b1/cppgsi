...
static const char *round_win_str(RoundWinReason r) {
    switch (r) {
        case RoundWinReason::CTWinElimination:
            return "CT win — elimination";
        case RoundWinReason::CTWinTime:
            return "CT win — time expired";
        case RoundWinReason::CTWinDefuse:
            return "CT win — defuse";
        case RoundWinReason::TWinElimination:
            return "T win — elimination";
        case RoundWinReason::TWinBomb:
            return "T win — bomb";
        default:
            return "unknown";
    }
...
static const char *map_phase_str(MapPhase p) {
    switch (p) {
        case MapPhase::Warmup:
            return "warmup";
        case MapPhase::Live:
            return "live";
        case MapPhase::Intermission:
            return "intermission";
        case MapPhase::GameOver:
            return "game over";
        default:
            return "unknown";
    }
...
int main() {
    ...
    dispatcher.map().on(&Map::phase, [](MapPhase old_p, MapPhase new_p) {
        std::cout << "[map phase] " << map_phase_str(old_p) << " -> " << map_phase_str(new_p) << "\n";
    });
    ...
    dispatcher.on_map([](const Map &old_m, const Map &new_m) {
        if (new_m.team_ct.name != old_m.team_ct.name && !new_m.team_ct.name.empty()) {
            std::cout << "[teams] CT: " << new_m.team_ct.name << " [" << new_m.team_ct.flag << "]"
                      << "  T: " << new_m.team_t.name << " [" << new_m.team_t.flag << "]\n";
        }
    });

    // Round outcome: fires at the end of each round when a new entry is appended.
    dispatcher.map().on(&Map::round_wins, [](const std::map<int, RoundWinReason> &old_wins,
                                             const std::map<int, RoundWinReason> &new_wins) {
        if (new_wins.size() > old_wins.size()) {
            auto it = new_wins.end();
            --it;
            std::cout << "[round " << it->first << "] " << round_win_str(it->second) << "\n";
        }
    });
    ...
    dispatcher.round().on(&Round::bomb, [](std::optional<BombState> old_b, std::optional<BombState> new_b) {
        if (new_b)
            std::cout << "[round.bomb] " << (old_b ? bomb_state_str(*old_b) : "none") << " -> "
    ...
    dispatcher.bomb().on(&Bomb::state, [](BombState old_s, BombState new_s) {
        std::cout << "[bomb] " << bomb_state_str(old_s) << " -> " << bomb_state_str(new_s) << "\n";
    });
    ...
    dispatcher.on_game_state([](const GameState &, const GameState &new_gs) {
        if (new_gs.previously && new_gs.previously->bomb) {
            ...
            std::cout << "[previously.bomb] was=" << bomb_state_str(was) << " now=" << bomb_state_str(now) << "\n";
        }
    });
    ...
    dispatcher.phase_countdowns().on(&PhaseCountdowns::phase_ends_in,
                                     [](double, double new_t) { std::cout << "[phase] " << new_t << "s remaining\n"; });
